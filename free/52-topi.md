...
<img src="../images/52-topi/01-ruby-girhub.png" width="100px" />
...
最悪計算量や平均計算量などについて知りたい方は下記の書籍がお勧めです。
...
今回は一番右から比較・並び替えを繰り返すとします。
まず、右から1番目と2番目を比較してみると、右から1番目が2、2番目が4で4>2なため入れ替えます。
<img src="../images/52-topi/03-01-bubble.png" width="400px" />

次に、右から2番目と3番目を比較してみると、右から2番目が先ほど入れ替えた2で、右から3番目が1であり、1<2が成り立つため入れ替えません。
<img src="../images/52-topi/03-02-bubble.png" width="400px" />

続いて、右から3番目と4番目を比較してみると、右から3番目が先ほど入れ替えなかった1で、右から4番目が3であり、3>1なため入れ替えます。
<img src="../images/52-topi/03-03-bubble.png" width="400px" />

続いて、右から4番目と5番目を比較してみると、右から4番目が先ほど入れ替えた1で、右から5番目が5であり、5>1なため入れ替えます。
<img src="../images/52-topi/03-04-bubble.png" width="400px" />

一番左まで来たので、1回目のソートはこれで終わりです。一番小さい1が一番左に来ましたね。
<img src="../images/52-topi/03-05-bubble.png" width="400px" />
残りの要素で同じ作業を繰り替えます。一番左=インデックス0は整列済みのためこれを排除しての並び替えです。
...
次に、左から2番目と左隣を比較します。5>3で3の方が小さいため、左隣と入れ替えます。3が1番左へ来たため3の操作は終了です。
<img src="../images/52-topi/05-03-insert.png" width="350px" />

つづいて、左から3番目と左隣=左から2番目を比較します。5>1と1の方が小さいため入れ替えます。
<img src="../images/52-topi/05-04-insert.png" width="350px" />

1にはまだ左があるため、さらに比較します。3>1で1の方が小さいため、左隣と入れ替えます。
...
左から4番目と左隣=左から3番目と比較します。5>4となるため入れ替えます。
<img src="../images/52-topi/05-07-insert.png" width="350px" />

4にはまだ左があるため、さらに比較します。3<4となり3の方が小さいため入れ替えません。4の操作はここで終了です。
...
ヒープ構造は簡単にいうと木構造で親は1~2の子を持つ状態です。今回は「根(親)は子より大きい」ヒープ構造とします。
まずこれまで通りの配列があるとします。
...
今回は要素数が7つで4:3で要素数が分かれるため、ここで全体要素数の半分が１度目の操作を終えたことになります。
...
まずは定義を確認してみましょう。書いてあるのはarray.cファイルです。
...
例えば`[4,1,6,2,0]`という配列があったとしてsortをメソッドを使うと`[4,1,6,2,0].sort`といった使い方をします。
...
util.cの208行目以降にあります。行数多すぎてここに書くのは憚られるのでGitHubのコードを片手に読んでください。
...
それ以外はutil.cの361行目にあります。コードが長いので書きません。
...
今回の記事を通して**言語やOSSの中身の覗き方**が伝われば良いなぁと思っています。画像が多くコードも多い記事になってしまい、何度も主催者であるMIDORIさんに「ページが！！ページが！！」と叫びました。この校閲や調整も苦労しそうな気がしています。お手数おかけします……。
